function [R2,B] = regress_template(session_dir,saveDir,templateType,runs,hemi,func,template,cluster,tcPart,leaveOut,V2V3)

% Run linear regression using a retinotopic template and movie correlation
% data
%
%   Usage:
%   [R2,B] = regress_template(session_dir,saveDir,templateType,runs,hemi,func,template,cluster,tcPart,leaveOut,V2V3)
%
...
% Choose if using V2-V3 correlations
if V2V3
...
if V2V3
    ...
    X = [corrV1V2,corrV1V3,mcorrV1V2,mcorrV1V3];
end
Y = movieMat(:);
[B,~,~,~,STATS] = regress(Y,X);
R2 = STATS(1);
