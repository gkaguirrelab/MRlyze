function [var_exp,V1inds,V2inds,V3inds] = calc_fine_template_varexp(session_dir,runs,hemi,template)

% Calculates the variance explained between a template correlation matrix, 
% for each vertex on the decimated surface.
%
%   Usage:
%   [var_exp,V1inds,V2inds,V3inds] = calc_template_varexp(session_dir,runs,hemi,template)
%
...
for i = 1:full_length
    ...
    var_exp(1,i) = (corr(tmpV1mov,tmpV1corr))^2;
    var_exp(2,i) = (corr(tmpV2mov,tmpV2corr))^2;
    var_exp(3,i) = (corr(tmpV3mov,tmpV3corr))^2;
    var_exp(4,i) = (corr(tmpMov,tmpCorr))^2;
end
%% Save as .mat file
save(fullfile(pRF_dir,[hemi '.' template '.varexp.mat']),'var_exp','V1inds','V2inds','V3inds');