function plot_template_residual(session_dir,subject_name,hemi,template,makeplot)

% Plots the output of 'calc_template_residual' on the cortical surface
%
%   Usage:
...
load(fullfile(pRF_dir,[hemi '.' template '.residualSS.mat']));
%% Plot on surface
...
for i = 1:4
    tmp.vol(V1ind) = residualSS(i,V1inds);
    tmp.vol(V2ind) = residualSS(i,V2inds);
    tmp.vol(V3ind) = residualSS(i,V3inds);
    inFile = fullfile(pRF_dir,[hemi '.' template '.residualSS.' names{i} '.0.1.inflated.nii.gz']);
    outFile = fullfile(pRF_dir,[hemi '.' template '.residualSS.' names{i} '.inflated.nii.gz']);
