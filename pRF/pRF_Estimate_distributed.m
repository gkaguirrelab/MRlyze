...
%% estimate pRFs
for voxNum = 1:length(vtc)
    lockfile = opt.lockfile(voxNum);
    donefile = opt.donefile(voxNum);
    errorfile = opt.errorfile(voxNum);
    if exist(lockfile,'file')
        continue
    else
        fid = fopen(lockfile,'w');fclose('all'); % make lockfile
        try
            %for voxNum = 1:length(vtc)
            ...
            fid = fopen(donefile,'w');fclose('all'); % make donefile
        catch e
            fid = fopen(errorfile,'w');fclose('all'); % make error file
            save(errorfile,'e');
            throw(e); % Throw error on the screen
        end
    end
end
